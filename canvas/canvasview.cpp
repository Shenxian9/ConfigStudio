...
void CanvasView::hookupItem(CanvasItem *item)
{
    if (!item)
        return;
...
CanvasItem* CanvasView::addItem(const QString &type, const QRect &geometry, const QString &itemId)
{
    ...
    if (!itemId.trimmed().isEmpty())
        item->setObjectName(itemId.trimmed());
    QRect target = geometry;
    if (target.width() <= 0 || target.height() <= 0) {
        const QSize fallback = item->size().isValid() ? item->size() : QSize(140, 100);
        target.setSize(fallback);
    }
...
QList<CanvasItem*> CanvasView::items() const
{
    return findChildren<CanvasItem*>(QString(), Qt::FindDirectChildrenOnly);
}
...
void CanvasView::dropEvent(QDropEvent *event)
{
    ...
    CanvasItem *item = addItem(type, QRect(event->pos(), QSize()));
